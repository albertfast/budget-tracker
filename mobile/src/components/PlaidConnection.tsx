...
export default function PlaidConnection({
  ...
  const createLinkToken = async (retryAttempt = false) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!authService.isAuthenticated()) {
        console.log("User not authenticated for Plaid Link, attempting login...");
        await authService.login();
      }

      console.log("[Plaid] Creating link token...");
      const response = await makeAuthenticatedRequest(
        `${API_BASE_URL}/api/v1/banks/link/create-token`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        // If 401/403, try to re-login and retry ONCE
        if ((response.status === 401 || response.status === 403) && !retryAttempt) {
            console.log("Auth failed, attempting to re-login and retry...");
            await authService.login();
            return createLinkToken(true);
        }
      ...
      setError(
        "Failed to initialize Plaid Link. Please check your network connection."
      );
  ...
  if (error) {
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity onPress={() => createLinkToken(false)} style={styles.retryButton}>
